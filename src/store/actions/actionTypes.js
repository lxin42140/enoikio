/*----------------------fetch all listing----------------------*/
export const FETCH_LISTING_INIT = "FETCH_LISTING_INIT";
export const FETCH_LISTING_SUCCESS = "FETCH_LISTING_SUCCESS";
export const FETCH_LISTING_FAIL = "FETCH_LISTING_FAIL";

/*--------------------fetch filtered listing-----------------*/
export const FETCH_FILTERED_LISTINGS_INIT = "FETCH_FILTERED_LISTINGS_INIT";
export const FETCH_FILTERED_LISTINGS_SUCCESS =
  "FETCH_FILTERED_LISTINGS_SUCCESS";
export const FETCH_FILTERED_LISTINGS_FAIL = "FETCH_FILTERED_LISTINGS_FAIL";

export const DISPLAY_EXPANDED_LISTING = "DISPLAY_EXPANDED_LISTING";
export const RECORD_EXPANDED_LISTING = "RECORD_EXPANDED_LISTING";

/*----------------------Submit new listing----------------------*/
export const SUBMIT_NEW_LISTING_INIT = "SUBMIT_NEW_LISTING_INIT";
export const SUBMIT_NEW_LISTING_SUCCESS = "SUBMIT_NEW_LISTING_SUCCESS";
export const SUBMIT_NEW_LISTING_FAIL = "SUBMIT_NEW_LISTING_SUCCESS";

/*----------------------submit new listing photo----------------------*/
export const SUBMIT_NEW_PHOTO_INIT = "SUBMIT_NEW_PHOTO_INIT";
export const SUBMIT_NEW_PHOTO_SUCCESS = "SUBMIT_NEW_PHOTO_SUCCESS";
export const SUBMIT_NEW_PHOTO_FAIL = "SUBMIT_NEW_PHOTO_FAIL";

export const CLEAR_NEW_POST_DATA = "CLEAR_NEW_POST_DATA";

/*----------------------authentication----------------------*/
export const AUTH_START = "AUTH_START";
export const AUTH_SUCCESS = "AUTH_SUCCESS";
export const AUTH_FAIL = "AUTH_FAIL";
export const AUTH_LOGOUT = "AUTH_LOGOUT";

export const SET_AUTH_REDIRECT = "SET_AUTH_REDIRECT";

/*----------------------Chat----------------------*/
export const FETCH_CHAT_CONTACTS_INIT = "FETCH_CHAT_CONTACTS_INIT";
export const FETCH_CHAT_CONTACTS_SUCCESS = "FETCH_CHAT_CONTACTS_SUCCESS";
export const FETCH_FULL_CHAT_INIT = "FETCH_FULL_CHAT_INIT";
export const FETCH_FULL_CHAT_INIT_SUCCESS = "FETCH_FULL_CHAT_INIT_SUCCESS";
export const FETCH_FULL_CHAT_HISTORY_SUCCESS =
  "FETCH_FULL_CHAT_HISTORY_SUCCESS";
