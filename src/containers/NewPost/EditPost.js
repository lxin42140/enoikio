...
class EditPost extends Component {
	...
	onSubmitHandler = (event) => {
		...
		const postDetails = {
			...
			numberOfImages: this.state.imageAsFile.filter(image => image !== null).length,
		}
		...
		this.props.dispatchSubmitPhoto(this.state.imageAsFile, this.props.editListing.unique)
	...
	removeImageHandler = (imageName) => {
		...
		for (let image in imageArray) {
			if (imageArray[image] === imageName) {
				...
				const validForm = imageArray.filter(image => image != null).length !== 0;
...
const mapDispatchToProps = (dispatch) => {
	return {
		dispatchSubmitPhoto: (imageAsFile, identifier) =>
			dispatch(actions.submitEdittedPhoto(imageAsFile, identifier)),
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(withErrorHandler(EditPost, firebaseAxios));
